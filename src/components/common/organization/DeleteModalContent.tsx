...
const DeleteModalContent = ({ setShowModal, index }: any) => {
  ...
  const handleDeleteOrg = async () => {
    try {
      ...
      const response = await deleteOrg(userOrgDetails[index].id);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center justify-end p-5 pt-5  rounded-b gap-2.5">
        ...
        <button
          disabled={usernameValue !== userOrgDetails[index].username}
          className={`text-base w-full h-11 py-3 flex justify-center items-center bg-[#E60054] rounded-2xl font-medium text-white hover:bg-[#C20038] ${usernameValue !== userOrgDetails[index].username ? 'cursor-not-allowed' : ''}`}
          type="button"
